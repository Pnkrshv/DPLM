...
export default function Survey() {
    ...
    // Данные всех городов из API
    const [allCitiesData, setAllCitiesData] = useState({});
    const [allCitiesLoading, setAllCitiesLoading] = useState(false);

    // Доступные ФО и регионы из выбранного маршрута
    const [availableFOs, setAvailableFOs] = useState([]);
    const [availableRegionsByFO, setAvailableRegionsByFO] = useState({});

    // Состояния выбора
    const [selectedFOs, setSelectedFOs] = useState({});
    const [selectedRegionsMap, setSelectedRegionsMap] = useState({});
    const [expandedFOsResults, setExpandedFOsResults] = useState({});

    // Внутри export default function Survey()
    ...
    const fetchAllCities = async () => {
        ...
        try {
            ...
            setAllCitiesData(response.data);
        } catch (err) {
            console.error('Ошибка загрузки всех городов:', err);
        } finally {
    ...
    const handleSelectFO = (fo, checked) => {
        setSelectedFOs(prev => ({ ...prev, [fo]: checked }));
        setSelectedRegionsMap(prev => {
            const newMap = { ...prev };
            const regions = availableRegionsByFO[fo] || [];
            newMap[fo] = { ...newMap[fo] };
            regions.forEach(region => {
                newMap[fo][region] = checked;
            });
            return newMap;
        });
    };

    const handleSelectRegion = (fo, region, checked) => {
        setSelectedRegionsMap(prev => {
            const newMap = { ...prev };
            newMap[fo] = { ...newMap[fo], [region]: checked };
            return newMap;
        });
        // Обновляем состояние чекбокса ФО
        const allRegions = availableRegionsByFO[fo];
        const allChecked = allRegions.every(r =>
            selectedRegionsMap[fo]?.[r] || (r === region && checked)
        );
        setSelectedFOs(prev => ({ ...prev, [fo]: allChecked }));
    };

    const toggleExpandFO = (fo) => {
        setExpandedFOsResults(prev => ({ ...prev, [fo]: !prev[fo] }));
    };
    ...
    const resetResultsFilters = () => {
        ...
        // Сброс выбранных регионов
        setSelectedFOs({});
        setSelectedRegionsMap({});
    };
    ...
    const openResultsModal = async () => {
        ...
        // Заполняем список анкет для выбора
        setResultsQuestionnaires(questionnaires); // questionnaires уже загружены
        // Если анкета одна, сразу выберем её
        if (questionnaires.length === 1) {
            setResultsSelectedQuestionnaire(questionnaires[0].id);
        } else {
            setResultsSelectedQuestionnaire('');
        }

        // Убедимся, что список анкет загружен
        if (questionnaires.length === 0) {
            await fetchQuestionnaires();
        }
        setResultsQuestionnaires(questionnaires);
        if (questionnaires.length === 1) {
            setResultsSelectedQuestionnaire(questionnaires[0].id);
        } else {
            setResultsSelectedQuestionnaire('');
        ...
        // Определяем ФО и регионы, присутствующие в маршруте
        if (Object.keys(allCitiesData).length > 0) {
            const fosSet = new Set();
            const regionsByFO = {};

            cities.forEach(city => {
                const fo = city.district;
                const cityName = city.city;
                const regionsObj = allCitiesData[fo];
                if (regionsObj) {
                    for (const [region, citiesList] of Object.entries(regionsObj)) {
                        if (citiesList.includes(cityName)) {
                            fosSet.add(fo);
                            if (!regionsByFO[fo]) regionsByFO[fo] = new Set();
                            regionsByFO[fo].add(region);
                            break;
                        }
                    }
                }
            ...
            const fos = Array.from(fosSet).sort();
            setAvailableFOs(fos);
            const regionsObj = {};
            const selectedRegionsInit = {};
            const selectedFOsInit = {};
            fos.forEach(fo => {
                const regionsArray = Array.from(regionsByFO[fo]).sort();
                regionsObj[fo] = regionsArray;
                selectedRegionsInit[fo] = {};
                regionsArray.forEach(region => {
                    selectedRegionsInit[fo][region] = false;
                });
                selectedFOsInit[fo] = false;
            });
            setAvailableRegionsByFO(regionsObj);
            setSelectedRegionsMap(selectedRegionsInit);
            setSelectedFOs(selectedFOsInit);
        }
    };
                            ...
                            <div className="results-container">
                                ...
                                <div className="results-left">
                                    ...
                                    <div className="results-regions-block">
                                        ...
                                        <div className="results-region-list">
                                            {allCitiesLoading && <p className="loading-text">Загрузка...</p>}
                                            {!allCitiesLoading && availableFOs.length === 0 && (
                                                <p className="loading-text">Нет доступных регионов</p>
                                            )}
                                            {availableFOs.map(fo => {
                                                const isExpanded = expandedFOsResults[fo];
                                                const regions = availableRegionsByFO[fo] || [];
                                                const foSelected = selectedFOs[fo] || false;
                                                const anySelected = regions.some(r => selectedRegionsMap[fo]?.[r]);
                                                const indeterminate = anySelected && !foSelected;
                                                return (
                                                    <div key={fo} className="results-fo-item">
                                                        <div className="results-fo-header" onClick={() => toggleExpandFO(fo)}>
                                                            <p className="expand-icon">{isExpanded ? '▼' : '▶'}</p>
                                                            <input
                                                                type="checkbox"
                                                                checked={foSelected}
                                                                ref={el => { if (el) el.indeterminate = indeterminate; }}
                                                                onChange={(e) => handleSelectFO(fo, e.target.checked)}
                                                                onClick={(e) => e.stopPropagation()}
                                                            />
                                                            <p className="results-fo-name">{fo}</p>
                                                        </div>
                                                        {isExpanded && (
                                                            <div className="results-regions-list">
                                                                {regions.map(region => (
                                                                    <div key={region} className="results-region-item">
                                                                        <input
                                                                            type="checkbox"
                                                                            checked={selectedRegionsMap[fo]?.[region] || false}
                                                                            onChange={(e) => handleSelectRegion(fo, region, e.target.checked)}
                                                                        />
                                                                        <label>{region}</label>
                                                                    </div>
                                                                ))}
                                ...
                                <div className="results-right">
                                    <div className="results-toolbar">
                                        <div><button className="search-results-btn">Искать</button></div>
                                        <div className='toolbar-right'>
                                            <button className="results-action-btn">Поиск дублей</button>
                                            <button className="results-action-btn">Выгрузить для анализа</button>
                                            <button className="results-action-btn">Точки опроса</button>
                                            <button className="results-action-btn">Импорт из полог</button>
                                        </div>
                                    </div>

                                    <div className="results-survey-title">
                                        <h3>{surveyName || 'Опрос'}</h3>
                                    </div>

                                    <div className="results-questionnaire-selector">
                                        <label>Анкета</label>
                                        {resultsQuestionnaires.length === 1 ? (
                                            <span className="results-questionnaire-name">
                                                {resultsQuestionnaires[0]?.name || 'Без названия'}
                                            </span>
                                        ) : (
                                            <select
                                                value={resultsSelectedQuestionnaire}
                                                onChange={(e) => setResultsSelectedQuestionnaire(e.target.value)}
                                            >
                                                <option value="" disabled>Выберите анкету</option>
                                                {resultsQuestionnaires.map(q => (
                                    ...
                                    {/* Здесь в будущем будет отображаться таблица с результатами */}
                                    <div className="results-content-placeholder">
                                        <p>Результаты опроса по выбранной анкете появятся здесь</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div className="survey-window">
                        ...
                        <div className="survey-form">
                            <div className="step-contents" style={{ flex: 1, overflowY: 'auto' }}>
                                ...
                                {currentStep === 5 && (
                                    <div className="step-content">
                                        <h3 className='done_survey'>Опрос завершен!</h3>
                                    </div>
                                )}
                        ...
                        <div className="map-modal-body" ref={mapContainerRef} style={{ position: 'relative' }}>
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 750" className="russia-map-svg" ref={mapSvgRef}>
                                ...
                                <g className="district-labels">
                                    <text x="190" y="640" className="federal-district-label">Центральный</text>
                                    <text x="220" y="575" className="federal-district-label">Северо-Западный</text>
                                    <text x="415" y="560" className="federal-district-label">Южный</text>
                                    <text x="500" y="380" className="federal-district-label">Приволжский</text>
                                    <text x="600" y="310" className="federal-district-label">Уральский</text>
                                    <text x="760" y="360" className="federal-district-label">Сибирский</text>
                                    <text x="980" y="350" className="federal-district-label">Дальневосточный</text>
                                    <text x="350" y="630" className="federal-district-label">Северо-Кавказский</text>
                                </g>
                            </svg>
