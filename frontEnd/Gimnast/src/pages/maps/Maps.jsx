...
export default function Maps() {
    ...
    const handleSearchCity = async (cityName) => {
        ...
        try {
            ...
            alert("Ошибка при поиске города. Попробуйте ввести название на русском языке.");
        }
    };
    ...
    const handleEditRouteFromModal = () => {
        // Закрыть окно предпросмотра маршрута
        // setIsModalMapOpen(false);
        setIsEditingFromModal(true);
        // Сохраняем текущие данные маршрута
        const currentRouteName = routeName;
        const currentDescription = description;
        const currentCities = currentSavedCities;
        const currentEditingId = editingRouteId;

        // Преобразуем массив городов в структуру selectedCities
        const newSelectedCities = {};
        currentCities.forEach(({ district, city }) => {
            if (!newSelectedCities[district]) {
                newSelectedCities[district] = {};
            }
            newSelectedCities[district][city] = true;
        });

        // Устанавливаем состояния для открытия окна редактирования
        setSelectedCities(newSelectedCities);
        setSavedCities([]); // все города теперь в selectedCities
        setCurrentSavedCities(currentCities);
        setRouteName(currentRouteName);
        setDescription(currentDescription);
        setEditingRouteId(currentEditingId);
        setExpandedDistricts({}); // сбросить раскрытые регионы

        // Открыть окно создания/редактирования
    ...
    const handleSaveRoute = async () => {
        ...
        console.log("currentSavedCities:", currentSavedCities);

        const citiesList = currentSavedCities.length > 0 ? currentSavedCities : getSavedCities();

        console.log("citiesList:", citiesList);
        console.log("cities_count:", citiesList.length);

        const routeData = {
            name: routeName,
            description: description,
            cities_count: citiesList.length,
            cities_data: JSON.stringify(citiesList),
        };

        console.log("Отправляем на сервер:", routeData);
        ...
        try {
            ...
            alert("Ошибка при сохранении маршрута: " + (err.response?.data?.error || err.message));
        }
    };
    ...
    const loadRouteForEdit = async (routeId) => {
        try {
            ...
            try {
                const citiesData = JSON.parse(route.cities_data);
                setSavedCities(citiesData);
                setCurrentSavedCities(citiesData);
            } catch (e) {
    ...
    return (
        <>
                                    ...
                                    <ul className="suggestions-list">
                                        {suggestions && suggestions.length > 0 && suggestions.map((item, idx) => (
                                            <li
                                                key={idx}
                                                onClick={() => {
                                                    if (!item.city || !item.region || !item.district) {
                                                        alert("Некорректные данные о городе");
                                                        return;
                                                    }
                    ...
                    <div
                        ...
                        onClick={() => {
                            if (isEditingFromModal) {
                                // Если редактируем из предпросмотра – просто закрываем окно, не сбрасывая данные
                                setIsCreateMapOpen(false);
                                setIsEditingFromModal(false);
                            } else {
                                // Обычное создание – сбрасываем всё
                                setIsCreateMapOpen(false);
                                setRouteName("");
                                setDescription("");
                                setSavedCities([]);
                    ...
                    <div className="create-map-window">
                        <nav className="window-map-navigation">
                            ...
                            <div className="close-btn" onClick={() => {
                                setIsCreateMapOpen(false);
                                if (!isEditingFromModal) {
                                    // Если это не редактирование из предпросмотра – сбрасываем данные
                                    setRouteName("");
                                    setDescription("");
                                    setSavedCities([]);
                                    setCurrentSavedCities([]);
                        ...
                        <form className="create-map-form" onSubmit={(e) => {
                            ...
                            setCurrentSavedCities(getSavedCities());
                            ...
                            <div className="footer-map">
                                <button className="map-cancel" onClick={() => {
                                    if (isEditingFromModal) {
                                        setIsCreateMapOpen(false);
                                        setIsEditingFromModal(false);
                                    } else {
                                ...
                                <button className="map-submit" type="submit">Сохранить</button>
                            </div>
                        </form>
                    </div >
                </>
                                ...
                                <div className="main-modal-map">
                                    <div className="modal-map-buttons">
                                        ...
                                        <div className="settings-buttons">
                                            <button className="setting-discard" onClick={() => {
                                                setIsModalMapOpen(false);
                                                setRouteName('');
                                                setDescription('');
                                                setCurrentSavedCities([]);
                                            }}>Отклонить</button>
                                            <button className="setting-approve" onClick={() => {
                                                handleSaveRoute();
                                                setIsModalMapOpen(false);
                                            }
            ...
            <div className="buttons">
                ...
                <button className="update-btn" onClick={fetchRoutes}>
